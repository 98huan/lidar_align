...
std::ostream& operator<<(std::ostream& os,
  ...
  os << "\033[1m\033[36m"<< "t:[" << rigid.translation().x() <<","<<rigid.translation().y()<<"], yaw:["<<rigid.rotation().angle() <<"(" << rigid.rotation().angle() *180. /M_PI <<")]"<<"\033[0m";
  return os;
}
...
class Rigid3 {
 public:
  ...
  Eigen::Transform<FloatType,3,Eigen::Affine> ToAffine(){
    ...
    pose = Eigen::Translation<FloatType, 3>(translation_.x(),translation_.y(),translation_.z()) *rotation_;
    return pose;
  };
...
std::ostream& operator<<(std::ostream& os,
  ...
  os << "\033[1m\033[36m"<< "t:[" << rigid.translation().x() <<","<<rigid.translation().y()<<","<<rigid.translation().z()
    <<"]  quat:["<<rigid.rotation().w() <<","  <<rigid.rotation().x() <<"," <<rigid.rotation().y()<<"," <<rigid.rotation().z() <<"]"<<"\033[0m";
  return os;
}
...
template <typename FloatType>
Eigen::Matrix<FloatType, 3, 1> quatToEuler(const Eigen::Quaternion<FloatType>& rotation2){
  Eigen::Matrix<FloatType, 3, 1> euler_out(FloatType(0.0),FloatType(0.0),FloatType(0.0));
  Eigen::Matrix<FloatType, 3, 1> euler_out2(FloatType(0.0),FloatType(0.0),FloatType(0.0)); //second solution
  
  {
    ...
    Eigen::Matrix<FloatType, 3, 1> m_el[3];
    {
      ...
      FloatType xs = rotation.x() * s,   ys = rotation.y() * s,   zs = rotation.z() * s;
      FloatType wx = rotation.w() * xs,  wy = rotation.w() * ys,  wz = rotation.w() * zs;
      FloatType xx = rotation.x() * xs,  xy = rotation.x() * ys,  xz = rotation.x() * zs;
      FloatType yy = rotation.y() * ys,  yz = rotation.y() * zs,  zz = rotation.z() * zs;
      
      m_el[0] = Eigen::Matrix<FloatType, 3, 1>(FloatType(1.0) - (yy + zz), xy - wz, xz + wy);
      m_el[1] = Eigen::Matrix<FloatType, 3, 1>(xy + wz, FloatType(1.0) - (xx + zz), yz - wx);
      m_el[2] = Eigen::Matrix<FloatType, 3, 1>(xz - wy, yz + wx, FloatType(1.0) - (xx + yy));
    }
...
Rigid3<T> Interpolate(const TimestampedTransform<T>& start,
                      const TimestampedTransform<T>& end,
                      const double& time) {
